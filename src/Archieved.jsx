...
//     let result = dummy_data.filter((item)=>
...
useEffect(() => {
  const updateActivity = async (callId) => {
    const patchUrl = `https://cerulean-marlin-wig.cyclic.app/activities/${callId}`;
    ...
    try {
      const response = await fetch(patchUrl, {
        ...
        body: JSON.stringify({
          is_archived: true, // Or false, depending on the update you want to make
        }),
      });
    ...
    //   let result = updatedActivity?.filter((item)=>
    //   (item.is_archived === false))
    // setActivities(result);
  ...
  return (
    <>
      <div className="container">
        ...
        <h1 className="ml-5">Archived:{activities?.length} </h1>
        ...
        <div className="w-75 mt-5">
          {activities.map((item, index) => {
            ...
            return (
              ...
              //     <div className="card-body">
              //       <div className="date-label">{itemDate}</div>
              ...
              // </div>
            <div key={index} className="date-line mb-5">
              <Card item={item} handleArchive={handleArchive} />
            </div>
